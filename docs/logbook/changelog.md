...
Se centralizó la generación de PDF en el API con **pdfkit** (JS puro, sin navegador → cero dependencias de Chromium en Docker): la boleta/comprobante y el voucher de reserva se arman server-side en `modules/pdf` (`comprobante-pdf.ts`, `voucher-pdf.ts`, helpers en `format.ts`). Nuevo endpoint `GET /reservations/:id/comprobante.pdf`; el front (`printComprobante.ts`) solo descarga esos bytes. Se eliminaron los helpers huérfanos del front (`comprobante.ts`, `numeroALetras.ts`). Nota: se evaluó Puppeteer (HTML→PDF, fidelidad visual) pero se descartó por el riesgo de Chromium en Alpine ante deadline; pdfkit da un comprobante más simple pero garantiza el build de Docker. Habilita adjuntar el mismo PDF por correo (paso 2).
