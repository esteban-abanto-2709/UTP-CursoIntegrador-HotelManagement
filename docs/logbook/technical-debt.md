...
## [TD-028] Lógica de paginación por cursor duplicada en cada página
...
## [TD-029] El PDF (Puppeteer) necesita Chrome instalado; el Docker actual no lo trae

- **Ubicación:** `apps/api/Dockerfile`, `apps/api/src/modules/pdf/pdf.service.ts`
- **Riesgo:** 6/10
- **Problema:** Puppeteer descarga Chrome vía postinstall, pero pnpm no ejecuta build scripts por defecto, así que Chrome no queda en el contenedor. En local se resolvió corriendo `npx puppeteer browsers install chrome` (queda en `~/.cache/puppeteer`), pero el `Dockerfile` del API no instala Chrome ni sus librerías del SO. Al desplegar, todo endpoint que genere PDF (`GET /reservations/:id/comprobante.pdf` y el correo con adjunto) fallará con "Could not find Chrome".
- **Impacto futuro:** La generación de comprobantes y el envío de correos con PDF se caen en producción/Docker aunque funcionen en local. Solución al desplegar: en el `Dockerfile` instalar Chromium + deps (`apt-get install chromium ...`) y apuntar `PUPPETEER_EXECUTABLE_PATH`/`PUPPETEER_SKIP_DOWNLOAD`, o ejecutar `npx puppeteer browsers install chrome` en el build. Verificar tamaño de imagen resultante.
